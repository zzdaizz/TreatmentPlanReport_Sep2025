...
namespace PDFtoAria
{
	public class CustomInsertDocumentsParameter
	{
		...
		public static bool PostDocumentData(string patientId, VMS.TPS.Common.Model.API.User user, byte[] binaryContent, string templateName, DocumentType documentType)
		{
			...
			string docKey = "835b4a01-bd52-44ef-9b00-c2a771075438";// ConfigurationManager.AppSettings["docKey"];
		...
		public static string SendData(string request, bool bIsJson, string apiKey)
		{
			...
			{ UseDefaultCredentials = true, PreAuthenticate = true }))
			{
				...
				var hostName = "master-ae.vic.com";//ConfigurationManager.AppSettings.Get("hostName");
				var port = "55051"; //ConfigurationManager.AppSettings.Get("port");
				var dockey = "835b4a01-bd52-44ef-9b00-c2a771075438";//ConfigurationManager.AppSettings.Get("DocKey");
